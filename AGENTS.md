...
리뷰 범위는 **테스트 코드 + 제품 코드**이며, 코드 종류별로 역할을 나눈다.
- **테스트 코드** (`apps/web/tests/**`, `apps/web/playwright.config.ts`,
  `apps/web/vitest.config.ts`, `apps/back-office/tests/**` 등): 아래 "테스트 리뷰" 기준으로
  전면 검토.
- **제품 코드** (`apps/web/app/**`, `apps/**/src/**`, `packages/**/src/**` 등, 특히
  `apps/web/app/api/**` 같은 서버 라우트): 아래 "제품 코드 리뷰" 기준으로 **로직·보안·버그·회귀**만
  본다. 아키텍처/폴더 구조/네이밍/스타일 취향은 지적하지 않는다 — 그쪽은 `.coderabbit.yaml` 의
  path_instructions 담당이고, 구조는 추후 개편 예정이라 중복·소음만 된다.
...
## 제품 코드 리뷰 — 블로킹/주요 (로직·보안·버그만)
- **보안.** 인증·인가 우회, 비밀값·키 노출, 입력 검증 누락(외부 입력·LLM 응답·웹훅
  페이로드를 신뢰), 권한 경계가 약한 서버 라우트. `apps/web/app/api/**`·웹훅·외부 연동은
  항상 본다.
- **환경 게이팅.** 분석·추적·디버그·실험 코드가 운영/비운영 환경 구분 없이 동작하는 경우
  (호스트·`VERCEL_ENV` 등 가드 누락).
- **로직/회귀.** 명백한 버그, 경계 조건, null·예외 경로 누락, 기존 동작을 깨는 변경.
- **데이터.** 마이그레이션·쿼리의 의도치 않은 파괴적 변경, `returning()` 없이 결과에 의존,
  RLS/권한 정책 누락(신규 테이블은 deny-all 전제).
- 아키텍처·파일 배치·네이밍·포매팅·타입/린트는 **지적하지 않는다** (CodeRabbit·CI 담당).

## 자동 리뷰 출력 규약
- **출력은 항상 한국어.** 리뷰 범위·발견 결과(이슈 없음 포함)와 무관하게 본문·요약·인라인
  코멘트 전부 한국어로 쓴다.
- 인라인 코멘트, 파일·라인 명시. 실제 문제만. AI 생성 푸터·서명 금지.
- 블로킹/주요/마이너를 구분해 우선순위가 드러나게 한다.
- 도구가 강제하는 고정 헤더·배너(예: Codex 요약 배너 한 줄)는 도구 측 포맷이라 이 규약으로
  바꿀 수 없다 — 그 줄만 예외로 둔다.
