...
import {
  GenerateScenariosMultipartSchema,
...
/**
 * 시나리오 AI 생성. 요구사항·맥락 텍스트(또는 첨부 PDF/Markdown)를 받아 테스트 시나리오 배열을
 * 생성한다. 분석서 없이 시나리오만 만들며, 결과는 클라이언트에서 선택해 test_scenarios 로 저장한다.
 */

/** multipart 본문이 PDF 10MB / MD 1MB 한도 + overhead 를 넘으면 formData() 비용 자체를 피해 사전 거부. */
...
async function parseMultipartRequest(req: NextRequest): Promise<NormalizedInput> {
  // Content-Length 가 없거나 숫자가 아니면 본문 크기를 사전 판정할 수 없어 크기 가드가 무력화되므로 거부.
  ...
  const parsed = GenerateScenariosMultipartSchema.safeParse({
