...
export const validateProjectName = (pName: string): ValidationResult => {
  ...
  if (pName.trim().length > 50) return { isValid: false, error: PROJECT_NAME_ERRORS.MAX_LENGTH };
  return { isValid: true };
};
...
export const identifierValidation = (identifier: string): ValidationResult => {
  ...
  if (identifier.trim().length > 16) return { isValid: false, error: IDENTIFIER_ERRORS.MAX_LENGTH };
  return { isValid: true };
};
...
export const validateOwnerName = (ownerName?: string): ValidationResult => {
  ...
  if (ownerName.trim().length > 20) return { isValid: false, error: OWNER_ERRORS.MAX_LENGTH };
  return { isValid: true };
};
