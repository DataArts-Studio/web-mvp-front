...
export function detectChangedFields(oldData: SnapshotData, newData: SnapshotData): string[] {
...
export function generateChangeSummary(changedFields: string[], changeType: ChangeType): string {
...
export function computeFieldDiffs(
  ...
  return fieldMap.map(({ key, field }) => {
    ...
    return {
      ...
      oldValue:
        field === 'tags'
          ? Array.isArray(oldVersion[key])
            ? (oldVersion[key] as string[]).join(', ')
            : oldVal
          : oldVal,
      newValue:
        field === 'tags'
          ? Array.isArray(newVersion[key])
            ? (newVersion[key] as string[]).join(', ')
            : newVal
          : newVal,
      type,
    };
