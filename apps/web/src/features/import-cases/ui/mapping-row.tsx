...
export function MappingRow({ field, value, options, sampleRows, onChange }: MappingRowProps) {
  const samples = value ? sampleRows.map((row) => row[value] ?? '').filter(Boolean) : [];
  ...
  return (
    <div className="border-line-2 flex flex-col gap-2 border-b px-4 py-3 last:border-b-0 md:grid md:grid-cols-12 md:items-center md:gap-4">
      ...
      <div className="flex items-center gap-2 md:col-span-3">
        <span className="typo-caption1 md:typo-body2-normal text-text-1">{field.label}</span>
      ...
      {samples.length > 0 && (
        <div className="flex flex-col gap-0.5 md:col-span-5">
          {samples.map((sample, i) => (
            <p key={i} className="typo-caption1 text-text-3 truncate" title={sample}>
              {sample}
            </p>
          ))}
        </div>
