...
export async function getMilestoneById(
  ...
  try {
    ...
    const [dbMilestone] = await db.select().from(milestones).where(eq(milestones.id, milestoneId));
    ...
    const [relatedTestRuns, mtcRows, mtsRows] = await Promise.all([
      db
        ...
        .from(testRuns)
        .where(
          and(eq(testRuns.milestone_id, milestoneId), eq(testRuns.lifecycle_status, 'ACTIVE'))
        ),
      db
    ...
    const [allTestCaseRuns, directTestCases, suites, suiteCases] = await Promise.all([
      testRunIds.length > 0
        ? db
            .select({ test_case_id: testCaseRuns.test_case_id, status: testCaseRuns.status })
            .from(testCaseRuns)
            .where(
              and(inArray(testCaseRuns.test_run_id, testRunIds), isNull(testCaseRuns.excluded_at))
            )
        : Promise.resolve([]),
      mtcTestCaseIds.length > 0
        ? db
            .select({ id: testCases.id, case_key: testCases.case_key, name: testCases.name })
            .from(testCases)
            .where(
              and(inArray(testCases.id, mtcTestCaseIds), eq(testCases.lifecycle_status, 'ACTIVE'))
            )
        : Promise.resolve([]),
      suiteIds.length > 0
        ? db
    ...
    const testCaseMap = new Map<
      string,
      { id: string; caseKey: string; title: string; lastStatus: string | null }
    >();
    ...
    const completedCases = testCasesResult.filter(
      (tc) => tc.lastStatus && tc.lastStatus !== 'untested'
    ).length;
