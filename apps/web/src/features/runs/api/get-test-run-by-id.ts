...
async function repairTestRun(
  ...
  if (suiteIds.length > 0) {
    ...
    const existingRunSuites = await db
      .select({
        ...
        excluded_at: testRunSuites.excluded_at,
      })
      .from(testRunSuites)
      .where(eq(testRunSuites.test_run_id, runId));

    const existingSuiteIds = new Set(existingRunSuites.map((s) => s.test_suite_id));
    const excludedSuiteIds = new Set(
      existingRunSuites.filter((s) => s.excluded_at).map((s) => s.test_suite_id)
    );
    ...
    if (missingSuiteIds.length > 0) {
      await db
        ...
        .values(missingSuiteIds.map((suiteId) => ({ test_run_id: runId, test_suite_id: suiteId })))
        .onConflictDoNothing();
    }
    ...
    const suiteCaseRows =
      activeSuiteIds.length > 0
        ? await db
            .select({ id: testCases.id, test_suite_id: testCases.test_suite_id })
            .from(testCases)
            .where(
              and(
                inArray(testCases.test_suite_id, activeSuiteIds),
  ...
  if (msCaseIds.length > 0) {
    const existingMsCaseRuns = await db
      ...
      .from(testCaseRuns)
      .where(
        and(eq(testCaseRuns.test_run_id, runId), inArray(testCaseRuns.test_case_id, msCaseIds))
      );
...
export async function getTestRunById(testRunId: string): Promise<ActionResult<TestRunDetail>> {
  try {
    ...
    // 1. 테스트 실행 기본 정보
    const [run] = await db.select().from(testRuns).where(eq(testRuns.id, testRunId));
    ...
    if (extraSuiteIds.length > 0) {
      const extraSuites = await db
        .select({ id: testSuites.id, name: testSuites.name })
        .from(testSuites)
        .where(inArray(testSuites.id, extraSuiteIds));
    ...
    const sourceName =
      sourceNameParts.length > 0 ? sourceNameParts.join(' | ') : '직접 선택한 케이스';
    ...
    const testCaseRunDetails: TestCaseRunDetail[] = caseRuns.map((tcr) => {
      ...
      return {
        ...
        code: tc?.display_id ? `TC-${String(tc.display_id).padStart(3, '0')}` : tc?.case_key || '',
        title: tc?.name || '',
        status: tcr.status,
        comment: tcr.comment,
