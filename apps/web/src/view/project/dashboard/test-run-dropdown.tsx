...
export const TestRunDropdown = React.memo(
  ({ testRuns, selectedRunId, onSelect, slug, selectedRunName }: TestRunDropdownProps) => {
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="bg-bg-3 border-line-2 rounded-4 shadow-3 absolute top-full right-0 z-20 mt-1 min-w-[240px] border py-1">
            {testRuns.map((run) => (
              <button
                key={run.id}
                onClick={() => {
