...
interface RunCaseDetailPanelProps {
  ...
  updateMutation: UseMutationResult<
    unknown,
    Error,
    { testCaseRunId: string; status: string; comment: string | null }
  >;
}
    ...
    <div className="flex w-[40%] min-w-0 flex-col overflow-hidden">
      {selectedCase ? (
        <>
          ...
          <div className="border-line-2 shrink-0 border-b px-5 py-3">
            <div className="mb-1 flex items-center gap-2">
              ...
              {selectedCase.executedAt && (
                <span className="text-text-4 ml-auto text-[11px]">
                  {new Date(selectedCase.executedAt).toLocaleString('ko-KR', {
                    month: 'short',
                    day: 'numeric',
                    hour: '2-digit',
                    minute: '2-digit',
            ...
            <h2 className="text-text-1 text-base leading-snug font-semibold break-all">
              {selectedCase.title || '제목 없음'}
            </h2>
          </div>

          {/* Status Buttons — inline 4-button row (fixed) */}
          <div className="border-line-2 relative shrink-0 border-b px-5 py-3">
            {updateMutation.isPending && (
              <div className="bg-bg-1/70 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-sm">
              ...
              {(['pass', 'fail', 'blocked', 'untested'] as const).map((status) => {
                ...
                return (
                  ...
                  >
                    ...
                    <kbd className="text-text-4 ml-0.5 hidden text-[10px] lg:inline">
                      {config.shortcut}
                    </kbd>
                  </button>
                );
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            <div className="border-line-2 flex flex-col gap-3 border-b px-5 py-3">
              ...
              {!selectedCase.preCondition &&
                !selectedCase.steps &&
                !selectedCase.expectedResult && (
                  <p className="text-text-4 py-2 text-xs">상세 정보가 등록되지 않았습니다.</p>
                )}
            </div>
            ...
            <div className="px-5 py-3">
              ...
              <textarea
                ...
                className="bg-bg-2 border-line-2 text-text-1 placeholder:text-text-4 focus:border-primary focus:ring-primary h-24 w-full resize-none rounded-lg border p-2.5 text-sm focus:ring-1 focus:outline-none"
              />
          ...
          <div className="border-line-2 bg-bg-1 flex shrink-0 items-center justify-between border-t px-5 py-2">
            ...
            <div className="flex gap-1">
              <button
                onClick={() => {
                  if (currentIndex > 0) setSelectedCaseId(filteredCases[currentIndex - 1].id);
                }}
                disabled={currentIndex === 0}
                className="rounded-1 text-text-3 hover:bg-bg-2 hover:text-text-1 p-1 transition-colors disabled:cursor-not-allowed disabled:opacity-30"
                aria-label="이전 케이스"
              >
...
const DetailSection = ({ title, content }: { title: string; content: string }) => (
  <div>
    ...
    <div className="bg-bg-2 rounded-lg px-3 py-2">
      <p className="text-text-1 text-sm leading-relaxed break-words whitespace-pre-wrap">
        {content}
      </p>
    </div>
  </div>
