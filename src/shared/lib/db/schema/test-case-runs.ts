...
export const testCaseRuns = pgTable('test_case_runs', {
    ...
    status: varchar('status', { length: 20 }).$type<TestCaseRunStatus>().default('untested').notNull(),
    comment: text('comment'),
    executed_at: timestamp('executed_at'),
    source_type: varchar('source_type', { length: 20 }).$type<TestCaseRunSourceType>().default('adhoc').notNull(),
