      ...
      <div className="flex flex-col gap-6">
        <header className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <div className="flex items-center gap-1 text-text-3 text-sm">
        ...
        {/* 태그 */}
        <div className='flex gap-2'>
          <h3 className="flex gap-1 items-center text-text-3">
        ...
        <div className='flex flex-col gap-4'>
          <div>
            <h3 className='text-lg mb-2 text-text-3 font-semibold'>전제 조건</h3>
            <div className='bg-bg-2 border border-line-2 rounded-4 p-4'>
          ...
          <div>
            <h3 className='text-lg mb-2 text-text-3 font-semibold'>테스트 단계</h3>
            <div className='bg-bg-2 border border-line-2 rounded-4 p-4'>
              <pre className="whitespace-pre-wrap">
                1. 이메일 입력 필드에 형식에 어긋나는 값(예: test@com, testId 등)을 입력한다. 2.
                비밀번호, 이름 등 나머지 필수 입력 필드에는 정상적인 값을 입력한다. 3. [회원가입
                완료] 버튼을 클릭한다.
              </pre>
            </div>
          </div>
          ...
          <div>
            <h3 className='text-lg mb-2 text-text-3 font-semibold'>예상 결과</h3>
            <div className='bg-bg-2 border border-line-2 rounded-4 p-4'>
              <pre className="whitespace-pre-wrap">
                1. 가입 요청이 차단되어야 하며, 다음 단계로 넘어가지 않아야 한다. 2. "올바른 이메일
                형식을 입력해주세요." 와 같은 명확한 에러 메시지가 노출되어야 한다. 3. 이메일
                입력란에 포커스가 가거나 붉은색 테두리 등으로 강조되어야 한다.
              </pre>
            </div>
          </div>
        </div>
        {/* 테스트 정보 */}
        <div className='flex gap-2'>
          <div className='flex-1 bg-bg-2 border border-line-2 rounded-4 p-4'>
        ...
        {/* 테스트 실행 */}
        <div className="flex gap-2">
          <DSButton className="flex-1 flex gap-2 items-center">
            <Play className='w-4 h-4'/>
            테스트 실행
          </DSButton>
          <DSButton variant="ghost" className="flex gap-2 items-center">
